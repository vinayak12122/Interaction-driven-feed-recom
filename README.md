...
<p align="center">
  <img src="./assets/laptop-view.png" width="65%" alt="Laptop View">
  <img src="./assets/mobile-view.png" width="25%" alt="Mobile View">
</p>
...
* **Exploration vs. Exploitation:** A hybrid ranking algorithm that balances "Smart" ML picks (60%) with "Discovery" items (40%) to prevent content echo chambers.
...
## Some High-Level System Designs

1. **Stateless Compute Pattern**, where the recommendation engine performs "Just-in-Time" training,it pulls raw interaction data and fits the Logistic Regression model in milliseconds only when a request is made.
2. To prevent the network from becoming a bottleneck, the architecture utilizes a **Hybrid Telemetry Strategy**, where "Likes" are treated as high-priority triggers that force an immediate re-calculation of the feed, "Skips" are silently buffered in a local bucket and sent in batches.
3. **Client-Side Asset Persistence** where Images are **Cached** inside Browser's own memory so latency could be less , and also looked for to call API as low as possible
...
* Each movie is represented as a 25-dimensional binary vector.
