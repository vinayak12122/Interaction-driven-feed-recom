...
const App = () => {
  ...
  const toggleLike = (index) => {
    ...
    setLikedIndex((prev) => {
      ...
      if (isAlreadyLiked) {
        return prev.filter((liked) => liked.id !== item.id);
      } else {
        return [...prev, { id: item.id, name: item.poster_url, category: item.genre_list }];
      }
  ...
  const syncWithBackend = async (forceRefresh = false) => {
    ...
    const hasInteractions = pendingBuffer.current.length > 0;
    const triggerThreshold = imgData.length - 5;
    const needsMoreData = forceRefresh || (currentIdx >= triggerThreshol && imgData.length > 0);
    ...
    try {
      ...
      if (needsMoreData){
        const limit = forceRefresh ? 20:30;
  ...
  const handleScroll = () => {
    ...
    if (newIdx !== currentIdx) {
      ...
      if (newIdx >= imgData.length - 5 && !syncInProgress.current) {
        syncWithBackend(true);
      }
    }
    // if (containerRef.current) {
      ...
      {deleteModel && (
        <div className='fixed inset-0 z-200 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4 '>
          ...
          >
            ...
            <p className="text-zinc-400 mb-6"> This will reset your feed and clear all your preferences. This action cannot be undone.
            </p>
